...
/**
 * The {@code PolynomialRegression} class performs a polynomial regression on an set of
 * <em>N</em> data points (<em>y<sub>i</sub></em>, <em>x<sub>i</sub></em>). That is, it fits a
 * polynomial <em>y</em> = &beta;<sub>0</sub> + &beta;<sub>1</sub> <em>x</em> +
 * &beta;<sub>2</sub> <em>x</em><sup>2</sup> + ... + &beta;<sub><em>d</em></sub>
 * <em>x</em><sup><em>d</em></sup> (where <em>y</em> is the response variable, <em>x</em> is
 * the predictor variable, and the &beta;<sub><em>i</em></sub> are the regression coefficients)
 ...
 * <p>
 * This implementation performs a QR-decomposition of the underlying Vandermonde matrix, so it
 * is neither the fastest nor the most numerically stable way to perform the polynomial
 * regression.
 *
...
public class PolynomialRegression implements Comparable<PolynomialRegression> {
	...
	/**
	 * Performs a polynomial reggression on the data points {@code (y[i], x[i])}. Uses n as the
	...
	public PolynomialRegression(double[] x, double[] y, int degree, String variableName) {
		...
		while (true) {
			...
			for (int i = 0; i < n; i++) {
				for (int j = 0; j <= this.degree; j++) { vandermonde[i][j] = Math.pow(x[i], j); }
			}
		...
		for (int i = 0; i < n; i++) { double dev = y[i] - mean; sst += dev * dev; }
