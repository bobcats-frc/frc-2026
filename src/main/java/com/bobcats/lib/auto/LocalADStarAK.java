...
public class LocalADStarAK implements Pathfinder {
	private final ADStarIO m_io = new ADStarIO();
	...
	public boolean isNewPathAvailable() {
		if (!Logger.hasReplaySource()) { m_io.updateIsNewPathAvailable(); }

		Logger.processInputs("LocalADStarAK", m_io);

		return m_io.isNewPathAvailable;
	}
	...
	public PathPlannerPath getCurrentPath(PathConstraints constraints, GoalEndState goalEndState) {
		if (!Logger.hasReplaySource()) { m_io.updateCurrentPathPoints(constraints, goalEndState); }

		Logger.processInputs("LocalADStarAK", m_io);

		if (m_io.currentPathPoints.isEmpty()) { return null; }

		return PathPlannerPath.fromPathPoints(m_io.currentPathPoints, constraints, goalEndState);
	}
	...
	public void setStartPosition(Translation2d startPosition) {
		if (!Logger.hasReplaySource()) { m_io.adStar.setStartPosition(startPosition); }
	}
	...
	public void setGoalPosition(Translation2d goalPosition) {
		if (!Logger.hasReplaySource()) { m_io.adStar.setGoalPosition(goalPosition); }
	}

	/**
	 * Set the dynamic obstacles that should be avoided while pathfinding.
	 *
	 * @param obs             A List of Translation2d pairs representing obstacles. Each
	...
	public void setDynamicObstacles(List<Pair<Translation2d, Translation2d>> obs, Translation2d currentRobotPos) {
		if (!Logger.hasReplaySource()) { m_io.adStar.setDynamicObstacles(obs, currentRobotPos); }
	}
