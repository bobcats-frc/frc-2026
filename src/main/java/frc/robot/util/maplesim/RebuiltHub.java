 ...
 *
 * <p>
 * This class simulates the <strong>HUB</strong>s on the field where <strong>FUEL</strong>s can
 * be scored. Whether it is active can be determined by using
...
public class RebuiltHub extends Goal {
	...
	protected static final Pose3d[] blueShootPoses = {
			new Pose3d(blueHubPose.plus(new Translation3d(0.5969, 0.447675, -0.5)),
					new Rotation3d(Degrees.of(0), Degrees.of(-15), Degrees.of(33.75))),
			new Pose3d(blueHubPose.plus(new Translation3d(0.5969, 0.149225, -0.5)),
	...
	public static final Pose3d[] redShootPoses = Arrays.stream(blueShootPoses)
			...
			.map((Pose3d toRotate) -> { return toRotate.rotateBy(new Rotation3d(Rotation2d.fromDegrees(180))); })
			.toArray(Pose3d[]::new);
	...
	protected void addPoints() {
		...
		arena.addPieceWithVariance(shootPose.getTranslation().toTranslation2d(),
				new Rotation2d(shootPose.getRotation().getZ()), shootPose.getMeasureZ(), MetersPerSecond.of(2),
				shootPose.getRotation().getMeasureY(), 0, 0.02, 15, 0.2, 5);
	}
