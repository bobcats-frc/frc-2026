...
// Fix for misplaced obstacles in MapleSim
...
public class Arena2026Rebuilt extends SimulatedArena {
	...
	public static final class RebuiltFieldObstaclesMap extends FieldMap {
		...
		public RebuiltFieldObstaclesMap(boolean AddRampCollider) {
			...
			// blue tower uprights
			addRectangularObstacle(UPRIGHT_X_LEN, UPRIGHT_Y_LEN,
					new Pose2d(UPRIGHT_OFFSET_FROM_END_WALL, UPRIGHT_OFFSET_FROM_SIDE_WALL, new Rotation2d()));
			addRectangularObstacle(UPRIGHT_X_LEN, UPRIGHT_Y_LEN, new Pose2d(UPRIGHT_OFFSET_FROM_END_WALL,
					UPRIGHT_OFFSET_FROM_SIDE_WALL + UPRIGHT_Y_SPACING, new Rotation2d()));

			// red tower uprights
			addRectangularObstacle(UPRIGHT_X_LEN, UPRIGHT_Y_LEN, new Pose2d(FIELD_X_MAX - UPRIGHT_OFFSET_FROM_END_WALL,
					FIELD_Y_MAX - UPRIGHT_OFFSET_FROM_SIDE_WALL, new Rotation2d()));
			addRectangularObstacle(UPRIGHT_X_LEN, UPRIGHT_Y_LEN, new Pose2d(FIELD_X_MAX - UPRIGHT_OFFSET_FROM_END_WALL,
					FIELD_Y_MAX - UPRIGHT_OFFSET_FROM_SIDE_WALL - UPRIGHT_Y_SPACING, new Rotation2d()));

			// blue trench wall
			addRectangularObstacle(TRENCH_WALL_X_LEN, TRENCH_WALL_Y_LEN,
					new Pose2d(TRENCH_WALL_OFFSET_FROM_END_WALL, TRENCH_WALL_OFFSET_FROM_SIDE_WALL, new Rotation2d()));
			addRectangularObstacle(TRENCH_WALL_X_LEN, TRENCH_WALL_Y_LEN, new Pose2d(TRENCH_WALL_OFFSET_FROM_END_WALL,
					FIELD_Y_MAX - TRENCH_WALL_OFFSET_FROM_SIDE_WALL, new Rotation2d()));

			// red trench wall
			addRectangularObstacle(TRENCH_WALL_X_LEN, TRENCH_WALL_Y_LEN,
					new Pose2d(FIELD_X_MAX - TRENCH_WALL_OFFSET_FROM_END_WALL, TRENCH_WALL_OFFSET_FROM_SIDE_WALL,
							new Rotation2d()));
			addRectangularObstacle(TRENCH_WALL_X_LEN, TRENCH_WALL_Y_LEN,
					new Pose2d(FIELD_X_MAX - TRENCH_WALL_OFFSET_FROM_END_WALL,
							FIELD_Y_MAX - TRENCH_WALL_OFFSET_FROM_SIDE_WALL, new Rotation2d()));
			...
			if (AddRampCollider) {
				...
				// blue hub + ramps
				addRectangularObstacle(HUB_X_LEN, HUB_Y_LEN + 2.0 * HUB_RAMP_LENGTH,
	 ...
	 *
	 * <p>
	 * This will create an Arena with the ramp areas marked as inaccessible. If you would like to
	 * change that use {@link #Arena2026Rebuilt(boolean)}. Additionally due to performance issues
	 * the arena will not spawn all fuel by default. If you would like to change this use
	 ...
	 *
	 * <p>
	 * Due to the nature of maple sim they can not be fully simulated and so either must be non
	 * existent or treated as full colliders. This behavior can be changed with the AddRampCollider
	 * variable. Additionally due to performance issues the arena will not spawn all fuel by
	 ...
	 *
	 * This method is useful for certain spawners like the return cutes on the hub to prevent the
	 * game pieces from being returned to the exact same position every time.
	 *
	 * @param info                       the info of the game piece
	 * @param robotPosition              the position of the robot (not the shooter) at the time of
	 *                                   launching the game piece
	 ...
	 *                                   launching the game piece, influencing the initial velocity
	 *                                   of the game piece
	 * @param shooterFacing              the direction in which the shooter is facing at launch
	 * @param initialHeight              the initial height of the game piece when launched, i.e.,
	 ...
	 * @param xVariance                  The max amount of variance that should be added too the x
	 *                                   coordinate of the game piece.
	 * @param yVariance                  The max amount of variance that should be added too the y
	 *                                   coordinate of the game piece.
	 * @param yawVariance                The max amount of variance that should be added too the
	 *                                   yaw of the game piece.
	 * @param speedVariance              The max amount of variance that should be added too the
	 *                                   speed of the game piece.
	 * @param pitchVariance              The max amount of variance that should be added too the
	 *                                   pitch of the game piece.
	 */
	public void addPieceWithVariance(Translation2d piecePose, Rotation2d yaw, Distance height, LinearVelocity speed,
			Angle pitch, double xVariance, double yVariance, double yawVariance, double speedVariance,
			double pitchVariance) {
		addGamePieceProjectile(new RebuiltFuelOnFly(
				piecePose.plus(new Translation2d(randomInRange(xVariance), randomInRange(yVariance))),
				new Translation2d(), new ChassisSpeeds(), yaw.plus(Rotation2d.fromDegrees(randomInRange(yawVariance))),
				height, speed.plus(MetersPerSecond.of(randomInRange(speedVariance))),
	...
	public void placeGamePiecesOnField() {
		...
		for (int x = 0; x < 12; x += 1) {
			for (int y = 0; y < 30; y += isInEfficiencyMode ? 3 : 1) {
				addGamePiece(new RebuiltFuelOnField(centerPieceBottomRightCorner
						.plus(new Translation2d(Inches.of(5.991 * x), Inches.of(5.95 * y)))));
			}
		}

		boolean isOnBlue = !DriverStation.getAlliance().isEmpty() && DriverStation.getAlliance().get() == Alliance.Blue;

		if (isOnBlue || !isInEfficiencyMode) {
			for (int x = 0; x < 4; x++) {
				for (int y = 0; y < 6; y++) {
					addGamePiece(new RebuiltFuelOnField(blueDepotBottomRightCorner
							.plus(new Translation2d(Inches.of(5.991 * x), Inches.of(5.95 * y)))));
				}
			}
		}
		...
		if (!isOnBlue || !isInEfficiencyMode) {
			for (int x = 0; x < 4; x++) {
				for (int y = 0; y < 6; y++) {
					addGamePiece(new RebuiltFuelOnField(redDepotBottomRightCorner
							.plus(new Translation2d(Inches.of(5.991 * x), Inches.of(5.95 * y)))));
				}
			}
		}
	 ...
	 * {@link #setShouldRunClock(boolean)}) is false.
	 *
	 ...
	 */
	public void setShouldRunClock(boolean shouldRunClock) { shouldClock = shouldRunClock; }
	 ...
	 *
	 * This function will dump up to 24 game pieces, dependent on how many game pieces are
	 * currently stored in the outpost. For more manual control of the game pieces in the outpost
	 * use {@link #outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)}. To have a human
	 * player attempt to throw a game piece into the hub use {@link #outpostThrowForGoal(boolean)}.
	 *
	 ...
	 *
	 * <p>
	 * This method comes with variance built in (to simulate human inconsistency) and will
	 ...
	 * player throws game pieces use
	 * {@link #outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)}
	 *
	 ...
	 *
	 * <p>
	 * This method comes with variance built in (to simulate human inconsistency). Additionally if
	 ...
	 */
	public void setEfficiencyMode(boolean efficiencyMode) { isInEfficiencyMode = efficiencyMode; }
	 ...
	 */
	public boolean getEfficiencyMode() { return isInEfficiencyMode; }
}
