 ...
 *
 * <p>
 * This class simulates a outpost on the field where fuel can be scored. It has a store of fuel
 * which can be added too by scoring in it. This fuel can then be dumped via {@link #dump()} or
 * {@link Arena2026Rebuilt#outpostDump(boolean)}, thrown at the goal via
 * {@link #throwForGoal()} or {@link Arena2026Rebuilt#outpostThrowForGoal(boolean)}, or thrown
 * with specific parameters via {@link #throwFuel(Rotation2d, Angle, LinearVelocity)} or
 * {@link Arena2026Rebuilt#outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)},
 */
public class RebuiltOutpost extends Goal {
	...
	public RebuiltOutpost(Arena2026Rebuilt arena, boolean isBlue) {
		super(arena, Centimeters.of(3), Inches.of(21), Centimeters.of(10), "Fuel",
				isBlue ? blueOutpostPose : redOutpostPose, isBlue, true);

		this.arena = arena;
		gamePieceCount = 24;

		StructPublisher<Pose3d> OutpostPublisher = NetworkTableInstance.getDefault()
				.getStructTopic("/SmartDashboard/MapleSim/Goals/" + (isBlue ? "BlueOutpost" : "RedOutpost"),
						Pose3d.struct)
				.publish();

		StructPublisher<Pose3d> OutpostThrowPublisher = NetworkTableInstance.getDefault()
				.getStructTopic("/SmartDashboard/MapleSim/Goals/" + (isBlue ? "BlueOutpostThrow" : "RedOutpostThrow"),
						Pose3d.struct)
				.publish();
		StructPublisher<Pose3d> OutpostDumpPublisher = NetworkTableInstance.getDefault()
				.getStructTopic("/SmartDashboard/MapleSim/Goals/" + (isBlue ? "BlueOutpostDump" : "RedOutpostDump"),
						Pose3d.struct)
				.publish();
	 ...
	 * therefore only hit about half the time. Additionally if the outpost does not have game
	 * pieces stored this method will not do anything. If you would like to manually control how
	 * the human player throws game pieces use
	 * {@link #outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)}
	 */
	public void throwForGoal() {
		throwFuel(isBlue ? Rotation2d.fromDegrees(45) : Rotation2d.fromDegrees(220), Degrees.of(75),
	...
	public void dump() {
		for (int i = 0; i < 24 && gamePieceCount > 0; i++) {
			gamePieceCount--;
			this.arena.addPieceWithVariance(isBlue ? blueDumpPose.toTranslation2d() : redDumpPose.toTranslation2d(),
					new Rotation2d(), Meters.of(1.7), isBlue ? MetersPerSecond.of(2) : MetersPerSecond.of(-2),
					Degrees.of(0), 0, 0.2, 5, 0.2, 5.0);
		}
	}
	 ...
	 * the outpost does not have game pieces stored this method will not do anything. If you would
	 * like to have the human player throw at the hub use {@link #outpostThrowForGoal(boolean)}
	 *
	...
	public void throwFuel(Rotation2d yaw, Angle pitch, LinearVelocity speed) {
		if (gamePieceCount > 0) {
			gamePieceCount--;
			arena.addPieceWithVariance(isBlue ? blueLaunchPose.toTranslation2d() : redLaunchPose.toTranslation2d(), yaw,
					Meters.of(1.7), speed, pitch, 0, 0, 15, 2, 5);
		}
	}
}
